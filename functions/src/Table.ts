...
export default class Table {
    ...
    constructor(html) {
        this.rooms = html
            .querySelectorAll("th")
            .filter(headerElement => headerElement.childNodes.length > 1)
            .map(data => ({
        ...
        for (const row of rows) {
            this.data.push([]);
            const time =
                row.querySelector("td").attributes["data-seconds"] / 3600;
            this.timeMap.push(time);
        }

        rows.map((row, rowInd) => {
            row.querySelectorAll("td")
                .filter(element => !element.classNames.includes("row_labels"))
                .map((data, colInd) => {
                    let offset = 0;
    ...
    getID(roomID: string, time: number) {
        ...
        return this.data[this.timeMap.indexOf(time)][col].bookingID;
    }
