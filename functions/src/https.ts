    ...
    (request, response) => {
        login(request.body)
            .then((res: { success: boolean }) => {
                response.status(200).send({ validCredentials: res.success });
            })
            .catch(err => {
                response.status(400).send({ validCredentials: false });
            });
    }
);
...
export const fetchSchedule = functions.https.onRequest(
    async (request, response) => {
        try {
            ...
            response
                .status(400)
                .send({ success: false, data: "Error fetching areas" });
        }
    }
);

export const bookSlot = functions.https.onRequest(async (request, response) => {
    ...
    if (session.success) {
        try {
            ...
            response
                .status(400)
                .send({ success: false, data: "Error booking slot" });
        }
    } else {
        response
            .status(400)
            .send({ success: false, data: "Invalid login credentials" });
    }
});
